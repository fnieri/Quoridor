...
json GameSetupSerializableMessageFactory::serializeGameSetup(GameMode gameMode, std::vector<std::string> &players)
{
    ...
    json setupJson
        = {{"domain", toJsonString(Domain::GAME_SETUP)}, {"action", toJsonString(GameSetup::CREATE_GAME)}, {"game_mode", toJsonString(gameMode)}, {"players", playerArray}};
    return setupJson;
}
...
json GameSetupSerializableMessageFactory::serializeGameCreationRequest(const std::string& sender, std::vector<std::string>& receivers, nlohmann::json configuration)
{
  json receiversArray = json::array();
  for (auto &player : receivers)
      receiversArray.push_back(player);

  json creationJson = {{"domain", toJsonString(Domain::GAME_SETUP)}, {"action", toJsonString(GameSetup::CREATE_GAME)}, {"sender", sender}, {"receivers",receiversArray}, {"configuration", configuration}};
