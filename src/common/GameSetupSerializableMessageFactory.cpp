...
json GameSetupSerializableMessageFactory::serializeGameSetup(GameMode gameMode, GameInvite gameInvite, std::vector<std::string> &players)
{
    ...
    json setupJson
        = {{"domain", toJsonString(Domain::GAME_SETUP)}, {"action", toJsonString(gameInvite)}, {"game_mode", toJsonString(gameMode)}, {"players", playerArray}};
    return setupJson;
}

json GameSetupSerializableMessageFactory::serializeGameSetup(
    GameMode gameMode, GameInvite gameInvite, std::vector<std::string> &players, int time, int increment)
{
    json playerArray = json::array();
    for (auto &player : players)
        playerArray.push_back(player);
