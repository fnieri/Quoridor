...
{
    json actionJson = {{"action", toJsonString(JsonPlayerAction::MOVE_PAWN)}, {"domain", toJsonString(Domain::IN_GAME_RELATED)}, {"move", pawnAction.serialized()}, {"player_id", playerID}};
    return actionJson;
}

json BoardActionsSerializableMessageFactory::serializeWallAction(WallAction wallAction, int playerID)
{
    json actionJson = {{"action", toJsonString(JsonPlayerAction::PLACE_WALL)}, {"domain", toJsonString(Domain::IN_GAME_RELATED)}, {"move", wallAction.serialized()}, {"player_id", playerID}};
    return actionJson;
}
